...
// "11 DEZ •••• 0461 Kiwify *Maf - Parcela 5/12 R$ 82,42"
const TX = /^(\d{1,2})\s+([A-ZÇa-zç]{3})\s+(?:[•\.\u2022]*\s*\d{4}\s+)?(.+?)\s+(?:−|-)?R\$\s*([\d.]+,\d{2})\s*$/;
const NEG = /(?:−|-)\s*R\$/;
const SKIP = /(Pagamento em|Saldo restante|RESUMO|TRANSA[ÇC][ÕO]ES|Total a pagar|Fatura anterior|Pagamento recebido|Total de compras|Limite|Fechamento|Saldo em aberto|pagamento m[ií]nimo)/i;

/** Fatura Nubank: vencimento "19 JAN 2026", período "11 DEZ a 11 JAN". */
export function parseNubankFatura(text: string): ParseResult {
  const warnings: string[] = [];

  const dueM = text.match(/Data de vencimento:\s*(\d{1,2})\s+([A-ZÇa-zç]{3})\s+(\d{4})/i);
  const totalM = text.match(/no valor de\s*R\$\s*([\d.]+,\d{2})/i) || text.match(/Total a pagar\s*R\$\s*([\d.]+,\d{2})/i);
  const closingM = text.match(/Fechamento da pr[óo]xima fatura\s*(\d{1,2})\s+([A-ZÇa-zç]{3})\s+(\d{4})/i);
  ...
  for (const raw of text.split(/\n/)) {
    ...
    if (!m) continue;
    const [, dd, monPt, desc, val] = m;
    const mm = monthPtToNum(monPt);
    if (!mm) continue;

    let amount = parseBRL(val);
    if (!Number.isFinite(amount) || amount === 0) continue;
    const isCredit = NEG.test(line); // valores com "−R$" são pagamentos/estornos
    ...
    if (isCredit) continue; // pagamentos da fatura anterior: reconciliação, não despesa
